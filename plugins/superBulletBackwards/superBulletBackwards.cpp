...
class superBulletBackwards : public bz_Plugin
{
    virtual const char* Name ()
    {
        return "Super Bullet Backwards";
    }
    virtual void Init(const char*);
	virtual void Event(bz_EventData*);
	~superBulletBackwards();
...
BZ_PLUGIN(superBulletBackwards)

void superBulletBackwards::Init(const char*) {
	Register(bz_eShotFiredEvent);
	Register(bz_ePlayerDieEvent);

	bz_registerCustomBZDBDouble("_superBulletDistance", 40);
}

superBulletBackwards::~superBulletBackwards() {}

void superBulletBackwards::Event(bz_EventData *ed)
{
	switch (ed->eventType)
	{
		case bz_eShotFiredEvent:
		{
			...
			float pos[3];
			pos[0] = playerRecord->lastKnownState.pos[0] - cos(playerRecord->lastKnownState.rotation)*bz_getBZDBDouble("_superBulletDistance");
			pos[1] = playerRecord->lastKnownState.pos[1] - sin(playerRecord->lastKnownState.rotation)*bz_getBZDBDouble("_superBulletDistance");
			...
			bz_setShotMetaData(shotGUID, "type", "SP");
