...
//
class rrTurret : public bz_Plugin
{
public:
  virtual const char* Name(){return "rrTurret";}
  ...
  int rrplayerinturret=-1;//the player in the turret. -1 for no one
  float firepos[3]={0,0,23};//the position from which to fire the turret
};

BZ_PLUGIN(rrTurret)

void rrTurret::Event(bz_EventData *eventData ){
  switch (eventData->eventType) {
    case bz_ePlayerUpdateEvent: {
      ...
      {//They are in the turret.
        ...
        if(rrplayerinturret != -1 && rrplayerinturret != playerID)
          {//If the last player in the turret was not ourselves, or no one. That means that two people are inside the turret
            if (playerSpawned(rrplayerinturret)) {//make sure the person still in the turret is not NR or dead
            ...
            else rrplayerinturret=playerID;//otherwise - if the player in the turret is not alive, or nr, then we set the playerID to ourself
          }
          else rrplayerinturret=playerID;//same - duplicate else.
      }
      else if(rrplayerinturret==playerID) {rrplayerinturret=-1;}
      //If the last player in the turret was us, but now we are not inside the turret, set the rrplayerinturret to no one (-1)

      //This is the eject button
      if(pointIn(pos,-10,10,-10,10,26,32) && rrplayerinturret != -1) {
        ...
        if (playerSpawned(rrplayerinturret)) {//If they are not dead, eject from the turret
          bz_killPlayer(rrplayerinturret,0,killer);//Kill player in the turret
          bz_sendTextMessage(BZ_SERVER,rrplayerinturret,message.c_str());//Send them a message
          bz_sendTextMessage(BZ_SERVER,playerID,"You ejected the tank out of the turret.");//Tell the ejector about it.
        }
        rrplayerinturret=-1;//reset the player in the turret - they've been ejected.
      }
    }break;

    case bz_ePlayerDieEvent: {
      bz_PlayerDieEventData_V2* dieData = (bz_PlayerDieEventData_V2*)eventData;
      if(dieData->flagKilledWith=="L" && rrplayerinturret != -1 && dieData->killerID != rrplayerinturret)
      {//If a player was killed by laser, change the killerID so the player in the turret gets a point.
        dieData->killerID=rrplayerinturret;
      }        
   }break;
